...
use crate::Credentials::BotUserCredentials::REDIRECT_URI;
use crate::Credentials::AccessScopes::get_all_scopes;
use crate::WebRequests::header::Header as WebRequestHeader;
use reqwest::{Client, Response};
use crate::ConsoleComponents::ConsoleComponents;
use crate::WebRequests::twitch::request_data;
use reqwest::header::{HeaderMap, HeaderValue};
use crate::Debug::fail_safely;
use crate::JSON::crawler::crawl_json;
use crate::Browser::Browser;
use crate::WebRequests::{request, is_html};
...
impl OauthToken {
    pub async fn request(web_client:&Client, client_id:ClientId, browser:&dyn Browser) -> OauthToken {
        ...
        if ! is_html(&received_response) {
            ...
            open::that(received_response.url().as_str());
        ...
        token = get_input_from_console("Upon authorizing your account, please post the URL of the page you are redirected to into the console to finalize authorization.");
    ...
    pub async fn validate<'life>(&mut self, client:&Client, components:ConsoleComponents<'life>) {
        ...
        let on_html_received = |string:String| { fail_safely("Expecting JSON!") };
        request_data(client,
                     VALIDATION_URL,
                     header_map,
                     on_string_received,
