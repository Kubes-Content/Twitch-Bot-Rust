...
impl ChannelPointsEvent {
    pub fn from_json(event_object:JsonObject) -> ChannelPointsEvent {
        ...
        let reward_user_input_required:bool = reward_object.get_string_property_value("is_user_input_required".to_string()).parse().unwrap();
        let reward_sub_only:bool = reward_object.get_string_property_value("is_sub_only".to_string()).parse().unwrap();

        let reward_image_object_option = reward_object.get_nullable_object_property("image".to_string());
        let reward_image_signature =
            match reward_image_object_option {
                None => { None },
                Some(reward_image_object) => {
                    let reward_image_url_1x = Url::from_str(reward_image_object.get_string_property_value("url_1x".to_string()).as_str()).unwrap();
                    let reward_image_url_2x = Url::from_str(reward_image_object.get_string_property_value("url_2x".to_string()).as_str()).unwrap();
                    let reward_image_url_4x = Url::from_str(reward_image_object.get_string_property_value("url_4x".to_string()).as_str()).unwrap();
        ...
        // reward object
        let reward_background_color = reward_object.get_string_property_value("background_color".to_string());
        let reward_enabled:bool = reward_object.get_string_property_value("is_enabled".to_string()).parse().unwrap();
        ...
        // reward object
        let reward_should_redemptions_skip_request_queue:bool = reward_object.get_string_property_value("should_redemptions_skip_request_queue".to_string()).parse().unwrap();

        // redemption object
        let redemption_user_input = redemption_object.get_string_property_value("user_input".to_string());
        let redemption_fulfilled = redemption_object.get_string_property_value("status".to_string()) == "FULFILLED";
        ...
        ChannelPointsEvent{
            timestamp,
            id: redemption_id,
            user: (redemption_user_id, redemption_user_login, redemption_user_display_name),
            channel_id,
            redeemed_at,
