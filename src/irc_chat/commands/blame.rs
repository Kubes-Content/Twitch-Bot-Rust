...
pub fn blame_random_user<TLogger>(_parser:DefaultMessageParser<TLogger>, message:TwitchIrcUserMessage, args:Vec<String>, context_mutex:Arc<tokio::sync::Mutex<ResponseContext>>, logger:&TLogger) -> Box<dyn Future<Output=()> + Unpin + Send>
    ...
    Box::new(Box::pin(async move {
        ...
        match context_mutex.try_lock() {
            Ok(mut context) => {
                let client_login = context.get_client_user().get_login();
                context.add_response_to_reply_with(send_message_from_client_user_format(client_login.clone(), blame_random_user_async(client_login).await));
            },
            Err(e) => { panic!("ERROR {}", e) },
        }
