use crate::{
    ...
    utilities::string_ext::{BeginsWith, Remove},
};
...
where
    TLogger: Logger + Clone,
{
    user_commands: HashMap<
        String,
        fn(
            Self,
    ...
    >,
    user_commands_alternate_keywords: HashMap<
        String,
        fn(
            Self,
            TwitchIrcUserMessage,
...
unsafe impl<TLogger> Send for DefaultMessageParser<TLogger> where TLogger: Logger + Clone {}

unsafe impl<TLogger> Sync for DefaultMessageParser<TLogger> where TLogger: Logger + Clone {}
    ...
    ) -> bool {
        {
            let response_received = {
                match context_mutex.try_lock() {
                    Ok(context) => context.get_initial_response().clone(),
                    Err(e) => panic!("ERROR: {}", e),
                }
            };
            ...
            if response_received.begins_with(TMI_TWITCH) {
                ...
                match self
                    .decipher_response_message(context_mutex.clone(), logger)
                    .await
                {
                    ...
                    Some(message) => {
                        match message {
                            ...
                            TwitchIrcMessageType::Message(message) => {
                                self.try_execute_command(message, context_mutex.clone(), logger)
                                    .await;
                            }
    ...
    TLogger: Logger + Clone,
{
    ...
    pub fn get_user_commands_including_alternates(
        &self,
    ) -> (
        HashMap<String, user_command_type!(), RandomState>,
        HashMap<String, user_command_type!(), RandomState>,
    ) {
        (
            self.user_commands.clone(),
    ...
    pub fn init_default_commands(&mut self) {
        ...
        self.add_command("lurk", vec!["afk", "busy"], enter_lurk);
        self.add_command("random", vec!["select"], random_selection);
        self.add_command("shoutout", vec!["shout", "so"], shoutout);
    ...
    pub fn add_command(
        ...
        command_ref: user_command_type!(),
    ) {
        self.user_commands
            .insert(primary_key.to_string(), command_ref);

        for alt_key in alternate_keys {
            self.user_commands_alternate_keywords
                .insert(alt_key.to_string(), command_ref);
        }
    }
    ...
    async fn decipher_response_message(
        ...
        context_mutex: Arc<tokio::sync::Mutex<ResponseContext>>,
        logger: &TLogger,
    ) -> Option<TwitchIrcMessageType> {
        let deconstructing_response;

        match context_mutex.try_lock() {
            Ok(context) => {
                if !context.get_initial_response().begins_with(":") {
                    return None;
                }
        ...
        if username_duplicate == None {
            ...
            match context_mutex.try_lock() {
                Ok(context) => {
                    if client_username_split.next()?.to_string()
            ...
            match response_whitespace_split.next()? {
                "353" => {
                    logger.write_line(String::from("Is this message only when the client joins? or when anyone joins a channel?"));
                }
                "366" => {
                    logger.write_line(String::from("End of names list.... (list only shows client's name atm) is this only after the client joins a channel?"));
                }
                _ => {
                    logger.write_line(String::from("??? Client Message"));
                }
            };
        ...
        match message_type {
            "PRIVMSG" => {
                ...
                Some(TwitchIrcMessageType::Message(TwitchIrcUserMessage::new(
                    username,
                    message,
                    channel_name,
                )))
            }
            "JOIN" => Some(TwitchIrcMessageType::JoiningChannel {
                joiner: username,
                channel: channel_name,
            }),
            _ => {
                logger.write_line(format!(
                    "Could not register IRC message type: {}",
                    message_type
                ));
                None
            }
    ...
    async fn try_execute_command(
            ...
            Ok(context) => {
                channel_id = {
                    if message.get_target_channel() != context.get_client_user_data().get_login() {
                        logger.write_line(
                            "TRYING TO EXECUTE COMMAND IN SOMEONE ELSE'S CHANNEL.".to_string(),
                        );
                        return false;
                    }
                ...
                if message.get_message_body().chars().next().unwrap() != '!'
        ...
        if let Some(command_func) = self.user_commands.clone().get(command.as_str()) {
            ...
            return true;
        } else if let Some(command_func) = self
            .user_commands_alternate_keywords
            .clone()
            .get(command.as_str())
        {
            command_func(
        ...
        let custom_commands = match CustomCommandsSaveData::load_or_default(channel_id) {
            Ok(commands_data) => commands_data,
            Err(e) => panic!("Error reading custom commands! Error: {}", e),
        }
        .get_commands();
        if custom_commands.contains_key(command.as_str()) {
            match context_mutex.try_lock() {
                Ok(mut context_mutex) => {
                    context_mutex.add_response_to_reply_with(send_message_from_client_user_format(
                        message.get_target_channel().clone(),
                        custom_commands.get(command.as_str()).unwrap().clone(),
                    ));
                }
                Err(e) => panic!("ERROR: {}", e),
            }
