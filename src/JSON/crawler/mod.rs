use crate::json::crawler::errors::Errors::JsonParseError;
...
pub fn crawl_json(json: &str) -> Result<json_object::JsonObject, Box<dyn Error>> {
    let mut current_scope: scope::Scope = scope::Scope::new();
    let mut progress_tracker: String = String::new();
    let mut character_handler: character_handler::CharacterHandler =
    ...
    for character in json.chars() {
        ...
        progress_tracker = format!(
            "{0}before char- '{1}' current context = MISSING\n",
        ...
        match character {
            ...
            '}' => {
                ...
                if current_scope.inside_parentheses {
                    ...
                    if current_scope.get_current_context()
                    ...
                    if current_scope
                        .reading_json_objects
                        .current_value_is_root_value()
                    {
                        return Ok(character_handler.register_final_closing(&mut current_scope));
                    }
